...
function loadDashboard(){
	fillSelectorsWithAttributes(false);
...
function fillSelectorsWithAttributes(includeValues) {// TODO atualizar quando filtrar ou selct

	var attrList = getKeys(data.attributes);

	for(var i = 0; i < attrList.length; i++) {// each attribute

		if(includeValues) {

			var values = data.attributes[attrList[i]];
			for(var j = 0; j < values.length; j++) {// each attr value  e.g. # boolean context  :  {"name" : [["yes", 24], ["no", 32]]}
				$('<option value="' + attrList[i] + '-' + values[j][0] + '">' + attrList[i] + ': ' + values[j][0] + '</option>').appendTo("#attr-y-1a");
			};

		} else {
			$('<option value="' + attrList[i] + '">' + attrList[i] + '</option>').appendTo("#attr-y-1a");
		}
	};
}
...
function changeOptionsChart(chartId, selectedOpt){
		alert("oi3")
}


function createStackedBarChartHightCharts(){
	
	alert("oi2")
	
	var concepts = data.nodes;

	var attr = $("#attr-y-1a option:selected").val();  // bird
	var attrValues = getAttributeValuesPairs(attr);
	...
	for (var i=0; i < attrValues.length; i++) {
		...
		entry.name = attrValues[i];
	    entry.data = [];
		
		series[attrValues[i]] = entry;
	...
	for (var i=0; i < concepts.length; i++) {
		...
		for (var j=0; j < attrValues.length; j++) {
		   ...
		   if (concepts[i].intent.indexOf( attrValues[j].split("-")[0])>= 0) {// TODO BOOLEAN attrs
	  			series[attrValues[j]].data.push(1);
	  			series[attrValues[j+1]].data.push(0);
	  			break;
	  		} else {
	  			series[attrValues[j]].data.push(0);
	  			series[attrValues[j+1]].data.push(1);
	  			break;
	  		}
		  
		};
	 
	  categories.push("concept "+ (i+1));
...
var p = 20,
    ...
    x = function(d) { return d.x * wi / mx; },
    y0 = function(d) { return he - d.y0 * he / my; },
    y1 = function(d) { return he - (d.y + d.y0) * he / my; },
    y2 = function(d) { return d.y * he / mz; }; // or `my` to not rescale
...
function transitionGroup() {
  ...
  function transitionEnd() {
    d3.select(this)
      .transition()
        .duration(500)
        .attr("y", function(d) { return he - y2(d); })
        .attr("height", y2);
  }
}
...
function getStackedBarSeries(items1, items2){
	...
	for (var i=0; i < items2.length; i++) {
		...
		for (var j=0; j < items1.length; j++) {
	  		var intersection = context.intersection(items1[i], items2[j])
